...
function renderSection(section, theme, isDark, index) {
    const controls = `
        <div class="section-controls">
            <button onclick="moveSection(${index}, -1)" ${index === 0 ? 'disabled' : ''}>↑</button>
            <button onclick="moveSection(${index}, 1)" ${index === sections.length - 1 ? 'disabled' : ''}>↓</button>
            <button onclick="editSection(${index})">✏️ Edit</button>
            <button class="delete-btn" onclick="deleteSection(${index})">🗑️ Delete</button>
        </div>
    `;
